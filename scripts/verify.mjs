...
export function verifyTasks(cpuCapacity, browserCapacity) {
  ...
  return [
    ...
    },
    {
      ...
      // given so much as take the ones that are there - which is why the number
      // it is charged is passed to it as `--maxWorkers` rather than left to it
      // to work out. A charge the child does not honour is a fiction.
      // See docs/decisions/The checks share the machine, and so do the tests.md.
    ...
    },
    {
      ...
      // goes to the other worker pools, so that between them the long tasks are
      // the machine and none has to guess what another took.
