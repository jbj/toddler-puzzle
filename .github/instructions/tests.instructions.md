...
that fails on a slow day. Each check counts the pieces as well as measuring
them, so a board that drew none would fail rather than pass by having nothing to
look at. Whether the join underneath is truly seamless is the one part only the
contact sheet can answer. See
