...
from that same box, so two waiting pieces cannot have overlapping grab areas
even where a piece is much smaller than the box it carries - which the clamp
never managed. The margin also follows the drawing rather than the authored box,
and still stops at the box's edge. See [One box measures a piece, and one rule
