# Rotation mode is not built, and the switch is gone
...
[Remember where the child stopped](<Remember where the child stopped.md>)),
...
into its neighbour's, against [Grab a piece anywhere in the box around its
artwork](<Grab a piece anywhere in the box around its artwork.md>). Whole
animals were the only pieces it clearly worked for, which is a narrow return for
...
a sound nobody hears, and [Sounds are data, and the machine
listens](<Sounds are data, and the machine listens.md>) is explicit that
`VOCABULARY` is the list of sounds somebody has looked at - an entry annotated
"unwired" that will never be wired is a standing invitation to wonder what is
broken. It is two lines of data if it is ever wanted again; this record and the
history are where to find them.
