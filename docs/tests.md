...
| `tests/grownups.test.ts` | The browser-free parts: the hold (state machine, clock passed in - 200 taps open nothing, near misses do not add up, the "Hold to open" prompt outlives the press); the map (thirty squares in five chapters of six, filled to `furthest`, one current from the game not the record); `toggleKind` pure and `isLastKindOn` refusing to empty the game. |
